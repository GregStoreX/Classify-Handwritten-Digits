...
time_pca = toc;
r = double(nominal(r));
x_train = transformed(training(par,1),:);
x_test = transformed(test(par,1),:);
r_test = r(test(par,1),:);
r_train = r(training(par,1),:);
...
save('pca_f64_cv_1_results.mat','B','dev','stats','time_pca','time_training','time_test','pihat','max_val','y','conf_mat','order');